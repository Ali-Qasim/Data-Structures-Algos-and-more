...
public class GameState {
    final char[] board;  //board represents the state space
    private char farmerPlace; //not really necessary. the same as board[BOARD_SIZE-1]
    static final char[] INITIAL_BOARD = {'L','L','L','L'};  //Initial state
    static final char[] GOAL_BOARD = {'R','R','R','R'};  //Goal state
    ...
    */
    ...
    public ArrayList<GameState> possibleMoves() {
        ArrayList<GameState> moves = new ArrayList<GameState>();  // Holds new states generated by 4 possible moves
        for (int start = 0; start < BOARD_SIZE-1; start++) {
            if (this.board[start] == this.farmerPlace) {  // If Corn, Goose, or Fox is on the same side as Farmer, they can move to the other side
                 GameState newState = this.clone();	  // New state to be generated by the move
        ...
        GameState newState = this.clone();  // New state to be generated with Farmer moving alone
	if (this.farmerPlace == 'L') {
	    newState.board[BOARD_SIZE-1] = 'R';
	    newState.farmerPlace = 'R';
	}  else {
